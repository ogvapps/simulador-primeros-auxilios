...
const AppContent = () => {
  ...
  if (view === 'admin') return <AdminPanel onBack={() => { setView('home'); setActiveTab('map'); }} s// Limpiar sesión del usuarioif (view === 'admin') return <AdminPanel onBack={() => { 
        // Limpiar sesión del usuario
        if (isMock && user) {
          const profileKey = 'pas_profile_' + user.uid;
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-800 dark:text-gray-100 flex flex-col font-sans transition-colors duration-300">
            ...
            <div className="flex-grow w-full relative flex flex-col" id="main-content">
              ...
              window.location.reload();<AdminPanel onBack={() => { 
              // Limpiar sesión del usuario
              if (isMock && user) {
                const profileKey = 'pas_profile_' + user.uid;
                localStorage.removeItem(profileKey);
